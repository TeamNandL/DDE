...
2. 2. Divorce is a POWER STRUGGLE between two leverages: one party holds the MONEY, the other holds the KIDS. Naming the game for a dad is blanket-useful, not legal advice.
3. 3. THE SLOW STARVATION (backbone emotional register): not the Hunger Games (quick, merciful) — it's a game of MILES not inches, slow meticulous starvation, eating yourself alive to survive. The tool must meet prolonged deprivation, not a single blow. Nick's anchor: chose to leave April 19; ~7 months without sleeping in the same house as his kids as of late Sept.
4. 4. The isolation hits IMMEDIATELY  the second a dad picks up the bag and walks out  not at month 6. The tool has to meet that immediate, total moment of need.
...
2. 2. NEVER DIAGNOSE THE EX'S MOTIVE: document behavior + effect on access only. Intent is a rabbit hole that pulls the dad back into the emotional story.
...
2. 2. PASTE vs LINK for chip_entry.
3. 3. VOICE: Grok runtime vs Nick's own ElevenLabs voice clone. The Grok distribution pivot conflicts with using his voice — Grok can't run a custom ElevenLabs voice. Pick convenience or his voice for v1. (Nick has a partial clone done — ~1 of 3 hours.)
4. 4. PROVISION "kids-facts 0/5" must be one question at a time, not a form.
5. 5. AUTH GAP flagged: no auth on the BFF — public template + public URL + dad_id = anyone can read/write any dad. Auth gate + RLS required before any real dad is provisioned.
6. 6. MULTI-AI question still open: Nick liked ChatGPT and Claude's voice; disliked Grok's voice feature. Whether/how to split platforms across components is undecided.
